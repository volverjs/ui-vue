...
- The `json-render` registry gives a component a default slot only when its element has children. A spec lists `children` on every element, `[]` for a leaf, as the schema requires and the prompt shows, and the registry turned that empty list into a default slot. A component that checks for one changed on it: an `Alert` with neither children nor content drew an empty content box, and a `Button` with an icon and no label never became icon only. An empty list now counts as no children. The `NamedSlots` story checks that its alert, which has neither, draws no content box, and the icon only button was checked with a probe, which found `vv-button--icon-only` missing before the change and present after it.
