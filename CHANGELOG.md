...
- `pnpm build` ends by checking the published declarations as an application would compile them. `scripts/declarations.js` imports each of the 45 subpaths of `exports` by its name, so the `types` of each is the file it resolves to, and compiles every declaration `./dist/*` publishes apart from those of the stories and the tests, with `skipLibCheck` off. It fails the build on any error TypeScript reports in a file of the package, leaving the declarations of the dependencies alone, and when `auto-imports.d.ts` declares global types again. It found four of the five defects in the first entry under Fixed, none of which shows from inside the repository, where the names they lack exist. TypeScript does not report every name it cannot resolve, one inside some type arguments for one, so the check catches what it reports.
...
- The published declarations resolve in an application. Compiled as a consumer compiles them, the declarations of 0.0.23 held 137 errors in 41 files. With `skipLibCheck` on, as most applications have it, the errors were not reported and the types they named silently became `any`, props included.
  ...
  - `icons.d.ts` copied the whole icon set, 37 KB, and imported three JSON files that are not in `dist`, so `detailed`, `normal` and `simple` were `any`. The collections are typed as `IconifyJSON`. The default export, whose type listed every icon, now has that type too, so a type derived from it, such as the union of the icon names, widens to `string`; `VvIcon` takes a string name either way.
...
- `VvNav` emits `click` when the click lands on markup inside an item. The listener on the list read the index of the item from the element clicked, which the `item` slot can fill with its own markup, so a click on it was lost: a `VvTab` with a `nav-item` slot did not switch tabs when a label drawn by the slot was clicked. The item is now looked up from the element clicked outwards, by the class of its link or button, never past the list, so markup with a `data-index` of its own is not taken for it. A `NavItemSlot` story on `VvTab` switches tabs with a click on such markup and from the keyboard.
- `v-contextmenu` binds its menu once. The directive binds on update, since the dropdown is usually a template ref still empty on the first render, and every later render of the host component added another `contextmenu` listener and another `scroll` listener, of which the unmount removed only the last. Only a different dropdown binds anew now, a dropdown that is removed leaves nothing listening for it, and one not rendered yet no longer throws. A `Rerender` story renders the host three times after the menu is bound and finds no listener added, then removes the dropdown and draws it again and finds its listener removed and added back once.
...
- `@json-render/core` and `@json-render/vue` move to 0.21.0 among the development dependencies, and the peer dependency floors stay at `>=0.19.0`. The release lists no breaking change, but it lets a Vue spec fill named slots, and `catalog.prompt()` now lists the named slots of the catalog and tells the model how to fill them: `summary` on `Accordion`, `header` and `footer` on `Card`, and `footer` on `Dialog` and `Alert`. The registry handed only `children` on, as the default slot, so the slots the prompt offered were dropped without a word. Those four components now also pass on the named slots json-render delivers in `slots`, while the default slot still comes from `children`, since json-render's `slots` always holds a default, children or not. Only the slots the catalog declares are passed on, since a spec can name any and json-render only warns about the others, and only when they list an element that exists, so an empty or not yet streamed list draws no empty footer. An element hidden by `visible` still opens its slot, since whether it shows is only known once it renders: a footer holding only hidden elements is drawn empty, as a default slot already was.
...
- `VvInputFile` lets the keyboard pick the file its drop area previews. With several files in a drop area, a click on an item of the list chose the one shown, and nothing else could: the items could not take the focus and listened to the mouse alone. While there is a choice to make they now take the focus, pick the file on Enter and Space and mark the chosen one with `aria-current`. The handler answers only keys pressed on the item itself, so the download and remove buttons inside it keep their own. A `SelectFromKeyboard` story, the first play test of the component, walks the list from the keyboard and runs the accessibility check.
- `VvAccordion` opens on mount when it has `not` and no model, which is what its documentation always said. The state started from `false` and `not` was read only by the watch on `modelValue`, and only for a boolean, so without a v-model that watch did nothing and `<VvAccordion not />` rendered closed, while `VvAccordionGroup` with `not` and no model already opened its first item on mount, or all of them with `collapse`. The state now starts from `not`. With a boolean model the immediate watch overwrites it before the watch that emits is registered, so `not` keeps inverting the model as before, and no `update:modelValue` leaves on mount with or without a model. A v-model bound to `undefined` counts as no model, and opens too.
  ...
  Inside a `VvAccordionGroup` the accordion still starts closed whatever its own `not` says, because the group owns the state. Starting from `not` there as well would have left the final state to the group, which toggles every accordion, but the accordion would have emitted `update:modelValue` every time the group closed it on arrival.
...
- `VvAccordionGroup` emits `update:modelValue` once for each operation, and only when the model changes. Every `toggle` wrote the model back through its setter, changed or not, and the setter emitted every time, so a group emitted once for each accordion on mount, every time with the value it already had: three accordions and `v-model="'a-1'"` emitted `'a-1'` three times, and in a `not` group each write handed the parent a new array holding the same names. A toggle that opens one accordion where only one can be open also closes the others, and the parent heard each step of it. The writes of a toggle, of the toggles sent on mount and of `expand` and `collapse` now make up one operation, and at its end the group compares the model with the one it started from, a string by value and an array by the names it holds, whatever their order, and emits the result once if they differ. The storage is written as before, so a `storageKey` still records the model the group mounted with.

  An application that listened to `update:modelValue` to learn the model on mount now hears it only when the group changes it, which an uncontrolled `not` group still does when it fills its list of closed accordions. A string model in a `collapse` group, which the group warns about, and an array in a group without `collapse` are still normalized by the first toggle, since the shape differs and the comparison sees it. The `LateItems` stories record every `update:modelValue` and find none for a model that never changes, and one for each change in the `not` group, and the `NotModel` story below hears the result of its mount once.
...
- `VvAccordionGroup` applies a model changed from outside. The group opened and closed its accordions only through the toggles of its bus, and nothing watched the model, so a parent that set `v-model` from `'a-1'` to `'a-3'` changed the model and nothing else: `a-1` stayed open and `a-3` closed. The group now watches the model it is given, or the storage when it is given none, deeply so that an array changed in place counts too, and when the value is not the echo of its own last write it sends every accordion the state the model gives it, the same toggles it sends on mount. A model that breaks the single open rule is resolved as on mount and the result is emitted once, while one that does not emits nothing, since it came from the parent. A parent that overrides the value inside its own `update:modelValue` handler gets that value applied, and one that ignores the event keeps what the group drew, as before. The order of an array model means nothing to the group, which writes the names in the order the accordions registered, so a parent that keeps them sorted, or a second group bound to the same model, is not taken for a change: compared in order, the two rewrote each other until Vue stopped them for recursion.

  `ExternalModel` and `ExternalModelNot` stories change the model from the parent and find the accordions following it, with nothing emitted. A `SortedModel` story sorts every update in the parent and finds nothing emitted on mount, and a single emit for a click.
...
- The `json-render` registry gives a component a default slot only when its element has children. A spec lists `children` on every element, `[]` for a leaf, as the schema requires and the prompt shows, and the registry turned that empty list into a default slot. A component that checks for one changed on it: an `Alert` with neither children nor content drew an empty content box, and a `Button` with an icon and no label never became icon only. An empty list now counts as no children, and so does the comment Vue hands on for an element with no `children` at all. The `NamedSlots` story checks that its alert, which has neither, draws no content box, and an `IconOnlyButton` story that its button gets `vv-button--icon-only`.
- The `Button` of the `json-render` catalog takes an `ariaLabel`, handed on to `VvButton`, and its description tells the model that an icon without a label needs one. The catalog gave no way to name a button that shows only an icon, which the fix above makes render as such, so one reached assistive technologies with no name at all. The `IconOnlyButton` story renders one with an `ariaLabel` and runs the accessibility check on it.
- The `json-render` catalog describes the `collapse` of `AccordionGroup` the right way round. It said that `collapse=true` allows only one accordion open at a time and closes the others when one opens, while the component does the opposite: without `collapse` one accordion is open at a time, with it the others stay open. A model building a UI from the catalog got the reverse of what it asked for. The JSDoc of `not` on `VvAccordionGroup`, which said that the items open by default, and the descriptions of `not` and `collapse` in the stories now say what the two props do.
- `VvAvatarGroup` keeps its avatars when it draws them again. An item without a `key` got one from `useUniqueId()`, called inside the computed that builds the list, so every time the list was computed again, on a change of `avatarModifiers`, `items` or `toShow`, each such item got a new key and Vue destroyed its avatar and created another, reloading its image. The key is now a prefix taken once in setup followed by the position of the item. A `StableKeys` story changes `avatarModifiers` and finds the same avatars, restyled in place.
