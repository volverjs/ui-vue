...
- `v-contextmenu` binds its menu once. The directive binds on update, since the dropdown is usually a template ref still empty on the first render, and every later render of the host component added another `contextmenu` listener and another `scroll` listener, of which the unmount removed only the last. Only a different dropdown binds anew now, and a dropdown not rendered yet no longer throws. A `Rerender` story renders the host three times after the menu is bound and finds no listener added.
