...
export async function defaultTest({ canvasElement, args }: PlayAttributes) {
    ...
    // `vv-radio` in @volverjs/style dresses with `flex: 1` and `min-width: 0`.
    ...
    const label = element.getElementsByClassName('vv-radio__label')[0]
...
export async function ariaLabelTest({ canvasElement }: PlayAttributes) {
    ...
    await expect(canvas.getByRole('radio', { name: 'Standard shipping' })).toBe(input)
